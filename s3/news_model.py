...
import re
# from check import post_title_list
...
def ndtv_movie_news_scraper():
    driver.get("https://food.ndtv.com/news")
    # data = post_title_list()
    ...
    news=driver.find_elements_by_xpath("//div[@class='lst-pg_txt-wrp']/a")
    for i in news:
    ...
    for post in post_list1:
        driver.get(post)
        heading=driver.find_elements_by_xpath("//h1[@class='sp-ttl']")
        paragraph=ndtv_movie_paragraph(post)
        paragraphs.append(paragraph)
        image=driver.find_elements_by_xpath("(//div[@class='ins_instory_dv_cont']/picture/img)[1]")
        for h in heading:
            headings.append(h.get_attribute('innerText'))
            categories.append('food news')
